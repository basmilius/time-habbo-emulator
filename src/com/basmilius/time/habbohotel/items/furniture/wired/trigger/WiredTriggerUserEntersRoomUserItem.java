...
public class WiredTriggerUserEntersRoomUserItem extends WiredTriggerUserItem
{

	public WiredTriggerUserEntersRoomUserItem(final ResultSet result) throws SQLException
	{
		super(result);
	}

	public WiredTriggerUserEntersRoomUserItem(final int habboId, final int roomId, final int itemId, final int catalogueItemId, final int itemBoughtBy, final BoughtType itemBoughtType, final int itemExpire)
	{
	...
	public boolean handle(final RoomUnit unit, final UserItem item, final String text)
	{
		...
		{
			final String specialUser = this.getValue("special_user", "");
			if (!specialUser.isEmpty())
				if (!specialUser.equalsIgnoreCase(unit.getName()))
					return false;
	...
	{
		packet.readInt();
		this.setValue("special_user", packet.readString());
	}
	...
	public void serializeWiredData(final ServerMessage response)
	{
		...
		response.appendString(this.getValue("special_user", ""));
		response.appendInt(0);
		response.appendInt(0);
