...
public class AdsBackgroundUserItem extends UserItem
{

	public AdsBackgroundUserItem(final ResultSet result) throws SQLException
	{
		super(result);
	}

	public AdsBackgroundUserItem(final int habboId, final int roomId, final int itemId, final int catalogueItemId, final int itemBoughtBy, final BoughtType itemBoughtType, final int itemExpire)
	{
	...
	public void serializeData(final ServerMessage response, final boolean isInventory, final boolean isDataOnly)
	{
		...
		if (this.getServerData().has("branding"))
		{
			Iterator<?> keys = this.getServerData().getJSONObject("branding").keys();

			while (keys.hasNext())
			{
				String key = (String) keys.next();

				data.put(key, (String) this.getServerData().getJSONObject("branding").get(key));
			}
		}

		response.appendInt(1);
		response.appendInt(data.size());
		for (Map.Entry<String, String> kv : data.entrySet())
		{
			response.appendString(kv.getKey());
			response.appendString(kv.getValue());
		}
	}
}
