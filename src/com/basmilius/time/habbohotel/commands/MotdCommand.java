...
@Command(
		executable = "cmd.syntax.motd",
		syntax = ":motd <message>"
)
public class MotdCommand extends ICommand
{

	public MotdCommand(RoomUnit invoker, boolean shouted, int bubbleId)
	{
	...
	public boolean handleCommand(String[] parts) throws Exception
	{
		if (((RoomUser) this.invoker).getHabbo().getPermissions().contains("cmd_ha"))
		{
			...
			Bootstrap.getEngine().getServer().getClientManager().sendBroadcastResponse(new MessagesOfTheDayComposer(new String[]{alert}));
